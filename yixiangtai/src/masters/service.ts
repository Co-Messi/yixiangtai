...
export async function getAIAnalysis(
  ...
  try {
    ...
    console.log('🚨 [DEBUG] 调用 API URL:', apiUrl.replace(effectiveApiKey, 'REDACTED_API_KEY'));
    const requestBody = {
      contents: [
        {
          parts: [
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response) {
        ...
        console.error('🚨 [DEBUG] 网络请求已发送但未收到响应 (error.request):', error.request);
        throw new Error(`网络连接失败 (${error.message || '请检查代理或网络'})`);
      }
    }
