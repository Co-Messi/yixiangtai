...
const NavItem: React.FC<NavItemProps> = ({ to, icon, label, isCollapsed, isActive, preloadKey }) => {
  ...
  return (
    <Link
      ...
      className={`
        relative flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ease-in-out overflow-hidden
    ...
    >
      {isActive && (
        <motion.span
          ...
          className="absolute inset-0 rounded-xl bg-[var(--ui-surface-2)] border border-[var(--ui-border)] shadow-[0_8px_18px_rgba(15,23,42,0.1)]"
          transition={{ type: 'spring', stiffness: 320, damping: 30 }}
        />
      )}
      <motion.span 
        className="relative z-10 flex-shrink-0 flex items-center justify-center w-5 h-5"
      ...
      <AnimatePresence>
        {!isCollapsed && (
          <motion.span 
            className="relative z-10 font-medium whitespace-nowrap ml-3 flex items-center"
