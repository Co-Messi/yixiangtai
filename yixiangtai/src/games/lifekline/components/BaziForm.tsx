...
const BaziForm: React.FC<BaziFormProps> = ({ onSubmit, isLoading }) => {
    ...
    return (
        ...
        <div className="w-full max-w-2xl mx-auto bg-[var(--ui-surface)] p-8 rounded-2xl border border-[var(--ui-border)]">
            <div className="text-center mb-8">
                <h2 className="text-2xl md:text-3xl font-semibold text-[var(--ui-text)] mb-2">精准八字排盘</h2>
                <p className="text-[var(--ui-muted-2)] text-sm">四柱推演 · 起运有据</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <label className="block text-sm font-medium text-[var(--ui-muted)] ml-1">姓名 (可选)</label>
                            <input
                                type="text"
                                name="name"
                                value={formData.name}
                                onChange={handleChange}
                                className="w-full px-4 py-3 bg-[var(--ui-surface-2)] border-2 border-[var(--ui-border)] text-[var(--ui-text)] rounded-xl focus:border-[var(--ui-accent)] outline-none transition-all placeholder:text-[var(--ui-muted-2)] text-base"
                                placeholder="输入姓名以生成报告"
                            />
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-[var(--ui-muted)] ml-1">性别 (决定大运方向)</label>
                        <div className="flex bg-[var(--ui-surface-2)] rounded-xl p-1 border-2 border-[var(--ui-border)] h-[112px] flex-col gap-1">
                            {(['男', '女'] as const).map(g => (
                                <button
                                    key={g}
                                    type="button"
                                    onClick={() => setFormData({ ...formData, gender: g })}
                                    className={`flex-1 rounded-lg text-base font-semibold transition-all duration-300 ${formData.gender === g
                ...
                {/* Four Pillars */}
                <div className="bg-[var(--ui-surface-2)] p-6 rounded-2xl border border-[var(--ui-border)] space-y-6">
                    <div className="flex items-center gap-3 text-[var(--ui-accent)] text-sm font-semibold mb-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="md:col-span-2 space-y-3">
                            <label className="block text-xs font-medium text-[var(--ui-muted)] ml-1 flex justify-between items-center">
                                <span>出生日期及精确时间 (公历)</span>
                                <span className="text-[10px] text-[var(--ui-muted-2)]">DD / MM / YYYY</span>
                            </label>
                            <div className="relative group">
                                <input
                                    ...
                                    onChange={handleChange}
                                    className="w-full px-4 py-3 bg-[var(--ui-surface-2)] border-2 border-[var(--ui-border)] group-hover:border-[var(--ui-border)] text-[var(--ui-text)] rounded-xl focus:border-[var(--ui-accent)] outline-none font-medium text-base transition-all"
                                />
                                <Calendar className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[var(--ui-muted-2)] group-hover:text-[var(--ui-accent)] transition-colors" />
                            </div>
                            {selectionPreview && (
                                <div className="flex flex-wrap gap-3 px-4 py-2 bg-[var(--ui-accent)]/5 rounded-xl border border-[var(--ui-accent)]/10 animate-in fade-in duration-500">
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-[10px] font-semibold text-[var(--ui-muted-2)]">年</span>
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-[10px] font-semibold text-[var(--ui-muted-2)]">月</span>
                                        <span className="text-sm font-bold text-[var(--ui-text)]">{selectionPreview.month}月</span>
                                    </div>
                                    <div className="w-[1px] h-3 bg-[var(--ui-border)] self-center" />
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-[10px] font-semibold text-[var(--ui-muted-2)]">日</span>
                                        <span className="text-sm font-bold text-[var(--ui-text)]">{selectionPreview.day}日</span>
                                    </div>
                                    <div className="w-[1px] h-3 bg-[var(--ui-border)] self-center" />
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-[10px] font-semibold text-[var(--ui-muted-2)]">时</span>
                            ...
                            <div className="text-[10px] text-[var(--ui-muted-2)] font-semibold mt-2">
                                排盘规则：传统子平（23:00后换日）
                            </div>
                        </div>
                        ...
                        {(['yearPillar', 'monthPillar', 'dayPillar', 'hourPillar'] as const).map(p => (
                            <div key={p} className="space-y-1.5">
                                <label className="block text-[11px] font-medium text-[var(--ui-muted)] ml-1 mb-1">
                                    {p === 'yearPillar' ? (
                                        <span className="flex justify-between">
                                            <span>年柱</span>
                                            <span className="text-[10px] text-[var(--ui-muted-2)]">属相基准</span>
                                        </span>
                                    ) : p === 'monthPillar' ? (
                                        <span className="flex justify-between">
                                            <span>月柱</span>
                                            <span className="text-[10px] text-[var(--ui-muted-2)]">依据节气</span>
                                        </span>
                                    ) : p === 'dayPillar' ? (
                                        <span className="flex justify-between">
                                            <span>日柱</span>
                                            <span className="text-[10px] text-[var(--ui-muted-2)]">核心元神</span>
                                        </span>
                                    ) : (
                                        <span className="flex justify-between">
                                            <span>时柱</span>
                                            <span className="text-[10px] text-[var(--ui-muted-2)]">归宿之位</span>
                                        </span>
                                    )}
                                </label>
                                <input
                                    ...
                                    placeholder="..."
                                    className="w-full px-4 py-3 bg-[var(--ui-surface-2)] border-2 border-[var(--ui-border)] text-[var(--ui-accent)] rounded-xl text-center font-black text-2xl shadow-inner cursor-default"
                                />
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-[var(--ui-surface-2)] p-6 rounded-2xl border border-[var(--ui-border)] space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-1.5">
                            <label className="block text-xs font-medium text-[var(--ui-muted)] ml-1">起运年龄</label>
                            <input
                                type="number"
                                name="startAge"
                                required
                                ...
                                onChange={handleChange}
                                className="w-full px-4 py-3 bg-[var(--ui-surface-2)] border-2 border-[var(--ui-border)] text-[var(--ui-text)] rounded-xl focus:border-[var(--ui-accent)] outline-none text-center font-black text-lg"
                            />
                        </div>
                        <div className="space-y-1.5">
                            <label className="block text-xs font-medium text-[var(--ui-muted)] ml-1">首步大运</label>
                            <input
                                type="text"
                                name="firstDaYun"
                                required
                                ...
                                onChange={handleChange}
                                placeholder="..."
                                className="w-full px-4 py-3 bg-[var(--ui-surface-2)] border-2 border-[var(--ui-border)] text-[var(--ui-accent)] rounded-xl text-center font-black text-xl"
                            />
                        </div>
                    </div>
                    <div className="pt-2 flex flex-col gap-2">
                        <div className="bg-[var(--ui-surface-2)] py-2 px-4 rounded-full border border-[var(--ui-border)] inline-block w-fit">
                            <p className="text-[10px] text-[var(--ui-muted)] font-semibold">
                        ...
                        {formData.luckStartDate && (
                            <div className="bg-[var(--ui-accent)]/10 py-2 px-4 rounded-full border border-[var(--ui-accent)]/20 inline-block w-fit">
                                <p className="text-[10px] text-[var(--ui-accent)] font-semibold">
                ...
                <button
                    type="submit"
                    disabled={isLoading}
                    className={`w-full py-4 rounded-xl font-semibold text-base transition-all duration-300 flex items-center justify-center gap-3 relative overflow-hidden group ${isLoading
                        ? 'bg-[var(--ui-surface-2)] text-[var(--ui-muted)] cursor-not-allowed'
                        : 'bg-[var(--ui-accent)] text-white hover:bg-[var(--ui-accent-strong)]'
                        }`}
                >
                    {isLoading ? (
                        <>
