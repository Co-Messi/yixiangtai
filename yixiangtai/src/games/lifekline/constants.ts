...
5. **K线数值范围**: \`open/close/high/low/score\` 必须是 0-100 的整数，不得出现负数或小数。
6. **K线幅度约束**: 每年 \`high - low\` 控制在 8-20 之间，\`|open - close|\` 控制在 2-12 之间；
   且 \`high >= max(open, close) + 2\`，\`low <= min(open, close) - 2\`。
7. **极值限制**: 避免过多接近 0 或 100 的数值，建议区间 \`low >= 5\`、\`high <= 95\`。
8. **巅峰年龄限制**: 巅峰年份不得固定在同一年龄（例如 43 岁），必须依据命盘信息变化。
9. **排盘规则**: 四柱已按传统子平排盘（23:00后换日）。
10. **输出格式**: 只输出严格JSON，不要Markdown/代码块/注释，字符串内不要换行与未转义的引号。
