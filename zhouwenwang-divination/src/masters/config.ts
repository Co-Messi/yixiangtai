...
/**
 * 获取 OpenAI API 密钥
 * 优先级：环境变量 > 用户设置中的 API 密钥
 * @param userApiKey 用户在设置中配置的 OpenAI API 密钥
 * @returns 最终使用的 OpenAI API 密钥
 */
export function getActiveOpenaiApiKey(userApiKey?: string): string {
  // 优先使用用户在设置中配置的API密钥 (UI优先)
  if (userApiKey && userApiKey.trim().length > 0) {
    return userApiKey.trim();
  }

  const envApiKey = API_CONFIG.OPENAI_API_KEY?.trim();
  if (envApiKey && envApiKey.length > 0) {
    return envApiKey;
  }
  return '';
...
export function hasValidApiKey(userApiKey?: string): boolean {
  ...
  return activeKey.length > 0; // Relaxed check for OpenAI and Gemini
} 
